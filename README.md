...
The console's country and region dropdowns read `leadbot/discovery/regions_map.json` — each key is a country, each value a list of cities/states. Regions are only geocoding hints: any city or state works even when it isn't listed (choose "Other (type manually)…" in the region dropdown). Extend the file to offer more presets, then restart the page.

## Files

- `app/` — CLI, configuration, progress, and pipeline orchestration
- `core/` — lead models and normalization, extraction, and scoring rules
- `discovery/` — geocoding, OpenStreetMap/Overpass search, and niche/region mappings
...
## V4 scoring and enrichment

V4 separates three decisions that were previously mixed together:

- **Contactability Score (0-100):** phone 40, email 40, social 20. A lead is contactable when it has at least one of these channels.
- **Lead Quality Score (0-100):** business/niche fit, website health/presence, location and source/data confidence. This score does not collapse to zero just because contact information is missing.
- **Sales Priority Score (0-100):** 50% lead quality + 30% contactability + 20% AI-agent fit. Priority bands are Hot (80+), High (65-79), Medium (50-64), and Nurture / Enrich (<50).

For no-website/no-contact leads, V4 runs a conservative enrichment cascade: OSM data -> Wikidata -> public-search website discovery -> homepage identity validation -> the normal website/contact extraction pipeline. Website discovery never bypasses robots, CAPTCHAs or authentication, and a candidate is accepted only after identity checks.

The default target is **70% contactability**. The target is used as an operating benchmark, not as a reason to relabel uncontactable leads. The rate should improve by recovering real public contact channels.
