...
class LeadBotHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if self.path == "/status":
            ...
            return
        self._send_html(page_html(last_query=self._load_last_query()))
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            self._send_html(page_html(last_query=self._load_last_query(), error_message=str(error)))
    ...
    @staticmethod
    def _load_last_query() -> dict[str, str] | None:
        """Return the most recently saved query so the form can pre-fill."""
        config_path = Path("config.json")
        if not config_path.exists():
            return None
        try:
            queries = json.loads(config_path.read_text(encoding="utf-8")).get("queries", [])
